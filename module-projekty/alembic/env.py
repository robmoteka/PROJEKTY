...
# Metadane zawierające wszystkie modele modułu Projekty — podstawa dla autogenerate
...
def include_name(name: str | None, type_: str, parent_names: dict) -> bool:  # type: ignore[override]
    """
    Filtr schematów dla autogenerate — Alembic zarządza wyłącznie schematem 'projekty'.

    Bez tego filtra Alembic skanuje WSZYSTKIE schematy w bazie (keycloak, shell, issues, public)
    i generuje destrukcyjne DROP TABLE dla tabel innych modułów. Filtr ogranicza
    inspekcję bazy wyłącznie do schematu 'projekty'.
    """
    if type_ == "schema":
        # Przepuść tylko schemat 'projekty' — ignoruj keycloak, shell, issues, public i inne
        return name == "projekty"
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # Tabela wersjonowania Alembic w schemacie projekty (nie public)
        version_table="alembic_version_projekty",
        version_table_schema="projekty",
