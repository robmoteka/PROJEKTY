...
# Metadane zawierające wszystkie modele modułu Issues — podstawa dla autogenerate
...
def include_name(name: str | None, type_: str, parent_names: dict) -> bool:  # type: ignore[override]
    """
    Filtr schematów dla autogenerate — Alembic zarządza wyłącznie schematem 'issues'.

    Bez tego filtra Alembic skanuje WSZYSTKIE schematy w bazie (keycloak, shell, public)
    i generuje destrukcyjne DROP TABLE dla tabel innych modułów. Filtr ogranicza
    inspekcję bazy wyłącznie do schematu 'issues'.
    """
    if type_ == "schema":
        # Przepuść tylko schemat 'issues' — ignoruj keycloak, shell, public i inne
        return name == "issues"
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # Tabela wersjonowania Alembic w schemacie issues (nie public)
        version_table="alembic_version_issues",
        version_table_schema="issues",
